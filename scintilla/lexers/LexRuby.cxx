...
static bool keywordIsModifier(const char *word,
                              int pos,
                              Accessor &styler);
static bool keywordIsDemoted(const char *prevWord);
...
static int ClassifyWordRb(unsigned int start, unsigned int end, WordList &keywords, Accessor &styler, char *prevWord) {
  ...
  if (0 == strcmp(prevWord, "class"))
    ...
    // Continue name searching when 'def self.' is written
    if (0 == strcmp(s, "self")) {
    ...
    //if (ALTERNATIVE_DEMOTED != 0) {
      if (keywordIsDemoted(s)) {
        chAttr = SCE_RB_WORD_DEMOTED;
      } else
        chAttr = SCE_RB_WORD;
    //} else if (keywordIsAmbiguous(s)
    ...
    //} else
    //  chAttr = SCE_RB_WORD;
  //} else if (isascii(s[0]) && isupper(s[0])) {
  } else if (isupper(s[0])) {
    k = 1;
    while ((k < lim) && (k < 70) && (isupper(s[k])))
      k++;
    if (k < lim)
      chAttr = SCE_RB_CONSTANT;
    else
      chAttr = SCE_RB_BIGCONSTANT;
  } else
...
static bool sureThisIsHeredoc(int iPrev,
    ...
    // to establish the context.  Not too likely though.
        ...
        case SCE_RB_IDENTIFIER:
            break;
        case SCE_RB_CONSTANT:
            break;
        case SCE_RB_BIGCONSTANT:
            break;
        default:
            return true;
        }
...
static bool sureThisIsNotHeredoc(int lt2StartPos,
    ...
    if ((prevStyle != SCE_RB_IDENTIFIER)
        ...
        && (prevStyle != SCE_RB_CONSTANT)
        && (prevStyle != SCE_RB_BIGCONSTANT)) {
          return definitely_not_a_here_doc;
    }
...
static bool keywordIsDemoted(const char *prevWord)
{
   if (!strcmp(prevWord, "nil")
     || !strcmp(prevWord, "true")
     || !strcmp(prevWord, "false")
     || !strcmp(prevWord, "self")
     || !strcmp(prevWord, "NIL")
     || !strcmp(prevWord, "TRUE")
     || !strcmp(prevWord, "FALSE"))
   {
